...
class MinHeap:
    ...
    def siftDown(self, currIdx, heap):
        ...
        while cOne is not None or cTwo is not None:
            childOneValue = float('inf')
            childTwoValue = float('inf')
            if cOne is not None:
                childOneValue = heap[cOne]
            if cTwo is not None:
                childTwoValue = heap[cTwo]

            if childOneValue < heap[currIdx] or childTwoValue < heap[currIdx]:
                minChild = cOne if childOneValue < childTwoValue else cTwo
                self.swap(currIdx, minChild, heap)
                currIdx = minChild
                cOne = self.childOne(currIdx, heap)
                cTwo = self.childTwo(currIdx, heap)
            else:
